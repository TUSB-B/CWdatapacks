...
### 追加スポナーから湧いたmobなら置き換え処理
execute if entity @s[tag=Spawn] run function tusb_cw:mob/replace/
...
execute if entity @s[tag=Enemy,nbt={Passengers:[{id:"minecraft:area_effect_cloud"}]}] on passengers if entity @s[tag=!TypeChecked] run data merge entity @s {Tags:[MobCloud,TypeChecked],Duration:28,Age:5,WaitTime:5,DurationOnUse:0,RadiusOnUse:0f,RadiusPerTick:0f,ReapplicationDelay:0}
