...
# Typeに応じてDisplayを変える
    execute if data storage debug:set_spawner _{wType:"Enemy"} run data modify storage debug:set_spawner _ merge value {CustomDisplayTile:1b,DisplayOffset:12,DisplayState:{Name:"minecraft:chorus_plant"}}
    execute if data storage debug:set_spawner _{wType:"Boss"} run data modify storage debug:set_spawner _ merge value {CustomDisplayTile:1b,DisplayOffset:15,DisplayState:{Name:"minecraft:redstone_torch"}}
    execute if data storage debug:set_spawner _{wType:"Friendly"} run data modify storage debug:set_spawner _ merge value {CustomDisplayTile:1b,DisplayOffset:10,DisplayState:{Name:"minecraft:oak_fence",Properties:{north:"true",south:"true",east:"true",west:"true"}}}
    execute if data storage debug:set_spawner _{wType:"Object"} run data modify storage debug:set_spawner _ merge value {CustomDisplayTile:1b,DisplayOffset:8,DisplayState:{Name:"minecraft:enchant_table"}}

# 湧き上限を99に設定(マーチャントとかで止められないように)
