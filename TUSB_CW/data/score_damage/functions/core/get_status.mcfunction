...
# 耐性エフェクト
    execute if data storage score_damage: Argument{BypassResistance:0b} store result score $Resistance ScoreDamageCore run data get entity @s active_effects[{id:"minecraft:resistance"}].amplifier
    execute if data storage score_damage: Argument{BypassResistance:0b} if data entity @s active_effects[{id:"minecraft:resistance"}] run scoreboard players add $Resistance ScoreDamageCore 1
