#> asset:mob/2441.escape_bonus_chest/data/2.data_set
...
### idを設定
    data modify storage asset: mob.id set value "ocelot"
...
### 体力等nbtを設定
    ...
    # 名前
        data modify storage asset: mob.CustomName set value '{"text":"逃げるボーナスチェスト"}'
    # 名前を表示するか
        data modify storage asset: mob.CustomNameVisible set value true
    # 死亡時のルートテーブル
        data modify storage asset: mob.DeathLootTable set value "usb:entities/escape_bonus_chest"
    # Tags
        # data modify storage asset: mob.Tags set value [CooldownRequired]
    # ポータルに入るまでのクールダウン。"CooldownRequired"というtagを付けているとこのnbtが0の時自動で消滅する
        # data modify storage asset: mob.PortalCooldown set value 1200
...
data modify storage asset: mob.Passengers[0] set value {\
    id: "block_display",\
    block_state: {\
        Name: "chest"\
    },\
...
data modify storage asset: mob.Passengers[1] set value {\
