#> asset:mob/2003.book_eater_middle/data/2.data_set
...
### 体力等nbtを設定
    ...
    # 名前
        data modify storage asset: mob.CustomName set value '{"text":"本を静かに貪る者 中巻"}'
    # 名前を表示するか
...
### Attributes
    ...
    # ノックバック耐性(0~1)
        data modify storage asset: mob.Attributes append value {Name:generic.knockback_resistance, Base:7}
    # 索敵範囲
...
### アイテム
    data modify storage asset: mob.HandItems set value [\
        {id:"minecraft:book",Count:1b,tag:{Enchantments:[{}]}},\
        {id:"minecraft:book",Count:1b,tag:{Enchantments:[{}]}}\
    ]
    ...
    data modify storage asset: mob.ArmorItems[].tag.display.color set value 4194508
...
### 本を貪る者のオーラ
    data modify storage asset: mob.Passengers append value {\
        ...
        Radius:1f,\
        Duration:30,\
        effects:[{id:"minecraft:slowness",amplifier:4b,duration:40}],\
