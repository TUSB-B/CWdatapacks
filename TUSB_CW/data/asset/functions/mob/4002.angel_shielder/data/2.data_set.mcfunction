#> asset:mob/4002.angel_shielder/data/2.data_set
...
### AssetIdを設定
    data modify storage asset: mob.AssetId set value 4002

### idを設定
    data modify storage asset: mob.id set value "zombified_piglin"
...
### 体力等nbtを設定
    ...
    # 名前
        data modify storage asset: mob.CustomName set value '{"text":"エンジェルシールダー"}'
    # 名前を表示するか
...
### active_effects
    ...
    #effect一覧
        ...
        # 詳しくはwiki見てね！
        # 効果時間を-1に設定すると無限になる
    data modify storage asset: mob.active_effects append value {id: "minecraft:jump_boost", amplifier: 4b, duration: -1, show_particles: 0b}
    data modify storage asset: mob.active_effects append value {id: "minecraft:slow_falling", amplifier: 0b, duration: -1, show_particles: 0b}

# 演出用AEC、
    data modify storage asset: mob.Passengers set value [\
        {id:"area_effect_cloud",duration:100,Particle:"end_rod",Radius:0.1},\
        {id:"potion",Item:{id:"minecraft:splash_potion",Count:1b,tag:{custom_potion_effects:[{id:"minecraft:instant_damage",amplifier:1b,duration:1},{id:"minecraft:hunger",amplifier:0b,duration:100}]}}}\
    ]