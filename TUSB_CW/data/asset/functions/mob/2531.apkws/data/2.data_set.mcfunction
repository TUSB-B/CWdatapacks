#> asset:mob/2531.apkws/data/2.data_set

# 最終的にPassengersやスポナーの中身に使える形になればそこまでがどのような形でもいい

# APKWS
# MH-60 Black Hawkから出現

# dataを初期化
    data remove storage asset: mob

# AssetIdを設定
    data modify storage asset: mob.AssetId set value 2531

# idを設定
    data modify storage asset: mob.id set value "phantom"
...
# 体力等nbtを設定
    ...
    # 名前
        data modify storage asset: mob.CustomName set value '{"text":"APKWS 誘導システム"}'
    # 名前を表示するか
    ...
    # Tags
        data modify storage asset: mob.Tags set value ["CooldownRequiredLong"]
    # DeathTime
    ...
    # ポータルに入るまでのクールダウン。"CooldownRequired"というtagを付けているとこのnbtが0の時自動で消滅する
        data modify storage asset: mob.PortalCooldown set value 600
    # 可読性や編集の手間を考慮しなければこれらを全て一つに纏めることも可能です

# Attributes
    # 最大体力
        data modify storage asset: mob.Attributes append value {Name:generic.max_health, Base:5}
    # (近接)攻撃力
...
# アイテム
    ...
    # 防具
        ...
        # 足
            data modify storage asset: mob.ArmorItems[0] set value {id: "tnt", Count: 65b, tag: {Enchantments: [{id: "blast_protection", lvl: 7}]}}
...
# spawner
    # 土台
        data modify storage asset: mob.Passengers append value {\
            ...
            ArmorItems: [{},{},{},{id: "tnt", Count: 65b, tag: {Enchantments: [{id: "blast_protection", lvl: 7}]}}]\
        }
    # 本体
        data modify storage asset: mob.Passengers[0].Passengers append value {\
            id: "spawner_minecart",\
            CustomDisplayTile: 1b, \
            DisplayState: {Name: "tnt"},\
            Tags: ["RidingRequired"],\
            Invulnerable: 1b,\
            Delay: 1s,\
            MinSpawnDelay: 1s,\
            ...
            SpawnData: {\
                ...
                entity: {\
                    ...
                    NoAI: 1b,\
                    Silent: 1b,\
                    active_effects: [{id: "invisibility", amplifier: 127b, duration: -1, ShowParticle: 0b}],\
                    ArmorItems: [{},{},{},{id: "tnt", Count: 65b, tag: {Enchantments: [{id: "blast_protection", lvl: 10}]}}],\
                    Passengers: [{\
                        id: "creeper",\
                        ExplosionRadius: 3b,\
                        Fuse: 0s,\
                        CustomName: '{"text":"APKWS"}'\
                    }]\
                }\
            }\
        }
    ...
    # data modify storage asset: mob.Passengers append value {\
        ...
        SpawnData: {\
            entity: {\
                ...
                effects: [{id: "instant_health", amplifier: 2b, duration: 2}],\
                Duration: 8s,\
                Age: 5s,\
                WaitTime: 5s,\
                ReapplicationDelay: 1s,\
                Radius: 1.0f,\
                Passengers: [{\
                    id: "area_effect_cloud",\
                    effects: [{id: "instant_health", amplifier: 2b, duration: 2}],\
