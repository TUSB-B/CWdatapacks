#> asset:mob/3611.kerberos/data/2.data_set
...
### AssetIdを設定
    data modify storage asset: mob.AssetId set value 3611

### idを設定
    data modify storage asset: mob.id set value "wither"

### 体力等nbtを設定
    # 体力
        data modify storage asset: mob.Health set value 900
    # 緩衝体力
    ...
    # 名前
        data modify storage asset: mob.CustomName set value '{"text":"§4ケルベロス"}'
    # 名前を表示するか
        # data modify storage asset: mob.CustomNameVisible set value true
    # 死亡時のルートテーブル
        data modify storage asset: mob.DeathLootTable set value "usb:entities/boss/kerberos"
    # Tags
        data modify storage asset: mob.Tags set value [Boss.Kerberos]
    # Team
...
### Attributes
    # 最大体力
        data modify storage asset: mob.Attributes append value {Name:generic.max_health, Base:900}
    # (近接)攻撃力
    ...
    # 防具
        # 頭
            data modify storage asset: mob.ArmorItems[3] set value {\
                ...
                Count:1b,\
                AttributeModifiers:[{}],\
                Enchantments:[{id:"minecraft:protection",lvl:10},{id:"minecraft:blast_protection",lvl:10}]\
            }
        # 胴
